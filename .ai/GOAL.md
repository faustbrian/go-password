...
The package MUST complement `go-authentication`; it MUST NOT manage users,
...
- `go-authentication` adapters for application user lookup without creating a
  reverse dependency from authentication core.
- `go-service` lifecycle and admission hooks.
- Secret-safe `go-log` and bounded `go-telemetry` observations.
- `go-postgres` examples for optimistic hash upgrades, not ORM ownership.
...
- `passwordauth`: optional integration with `go-authentication`.
