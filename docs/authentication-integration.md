...
reverse dependency from `go-authentication` or ownership of users.
...
4. Convert the returned stable subject into a `go-authentication.Principal`
...
The current `go-authentication` checkout is intentionally not imported because
it has no released module tag. This keeps `go-password` reproducible offline and
