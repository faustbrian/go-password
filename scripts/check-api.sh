...
current=$(mktemp "${TMPDIR:-/tmp}/go-password-api.XXXXXX")
report=$(mktemp "${TMPDIR:-/tmp}/go-password-api-report.XXXXXX")
...
go run "golang.org/x/exp/cmd/apidiff@${APIDIFF_VERSION}" -m -w "$current" github.com/faustbrian/go-password
